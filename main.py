...
def init_csv_log(log_path):
    ...
    with open(log_path, 'w', newline='') as csvfile:
        ...
        writer.writerow(['epoch', 'timestamp', 'train_loss', 'val_loss', 'val_acc', 'val_eer'])

def log_to_csv(log_path, epoch, train_loss, val_loss, val_acc, val_eer):
...
def evaluate_accuracy(dev_loader, model, device):
    ...
    with torch.no_grad():
        for batch_x, batch_y in tqdm(dev_loader):
            ...
            batch_scores = torch.softmax(batch_out, dim=1)[:, 1]  # Probability of spoof class
...
if __name__ == '__main__':
    ...
    csv_log_path = os.path.join(model_save_path, 'training_log.csv')
    ...
    for epoch in range(num_epochs):
        ...
        if epoch>=0:
            ...
            # Log to CSV file
            log_to_csv(csv_log_path, epoch, running_loss, val_loss, val_acc, val_eer)

            if val_eer < best_eer:
                best_eer = val_eer
                best_val_loss = val_loss
                patience_counter = 0
                # Save best model based on EER with comment in filename
                best_model_name = 'best_model_eer'
                if args.comment:
                    best_model_name = 'best_model_eer_{}'.format(args.comment)
                torch.save(model.state_dict(), os.path.join(
                    model_save_path, '{}.pth'.format(best_model_name)))
            else:
                patience_counter += 1
            ...
            print('\n{} - Train Loss: {:.6f} - Val Loss: {:.6f} - Val Acc: {:.2f}% - Val EER: {:.2f}%'.format(
                epoch, running_loss, val_loss, val_acc, val_eer*100))
            
            # Save epoch checkpoint with comment in filename
            epoch_model_name = 'epoch_{}'.format(epoch)
            if args.comment:
                epoch_model_name = 'epoch_{}_{}'.format(epoch, args.comment)
            torch.save(model.state_dict(), os.path.join(
                model_save_path, '{}.pth'.format(epoch_model_name)))
    ...
    print("Training completed. Best EER: {:.2f}%".format(best_eer*100))
    print("Training log saved to: {}".format(csv_log_path))
# CUDA_VISIBLE_DEVICES=0 python main.py --track=DF --lr=0.000001 --batch_size=16 --loss=weighted_CCE --num_epochs=100
# CUDA_VISIBLE_DEVICES=0 python main.py \
#   --track=LA \
#   --is_eval \
#   --eval \
#   --model_path=/root/autodl-tmp/SLSforASVspoof-2021-DF/models/Ablation/best_model_paper.pth \
...
#   --eval_output=/root/autodl-tmp/SLSforASVspoof-2021-DF/scores_LA_paper.txt