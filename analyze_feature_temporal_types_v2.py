"""Quick test with 10 samples to verify logic."""
import torch
...
print('='*60)
print('FULL ANALYSIS: 2000 samples for statistical significance')
print('='*60)
...
print(f'✓ Found {len(all_files)} total files, using first 2000 for analysis')
...
with torch.no_grad():
    for batch_x, _ in eval_loader:
        ...
        for b in range(sparse_features.shape[0]):
            ...
            count += 1
            if count % 100 == 0:
                print(f'  Processed {count}/2000 samples')
...
if discriminative_lifetimes and non_discriminative_lifetimes:
    ...
    t_stat, p_value = stats.ttest_ind(discriminative_lifetimes, non_discriminative_lifetimes)
    cohens_d = diff / np.sqrt((np.std(discriminative_lifetimes)**2 + np.std(non_discriminative_lifetimes)**2) / 2)
    ...
    if diff < 0:
        print('\n✅ Discriminative features ARE more transient (shorter lifetime)')
        print('   This supports the hypothesis!')
    else:
        print('\n❌ Discriminative features are MORE persistent (longer lifetime)')
    ...
    results = {
        ...
        'discriminative_features': {
            'mean_lifetime': float(np.mean(discriminative_lifetimes)),
            'median_lifetime': float(np.median(discriminative_lifetimes)),
            'num_instances': len(discriminative_lifetimes)
        },
        'non_discriminative_features': {
            'mean_lifetime': float(np.mean(non_discriminative_lifetimes)),
            'median_lifetime': float(np.median(non_discriminative_lifetimes)),
            'num_instances': len(non_discriminative_lifetimes)
        },
        'statistics': {
    ...
    with open('feature_temporal_types_analysis.json', 'w') as f:
        json.dump(results, f, indent=2)
    print('\n✅ Results saved to feature_temporal_types_analysis.json')
    
else:
    print('\n⚠️ Insufficient data
    print('\n⚠️ Insufficient data in 10 samples')
