#!/usr/bin/env python
"""
Evaluate CPC model on ASVspoof 2021 LA evaluation set - using torchaudio
"""
import argparse
import os
import sys
...
def evaluate_2021_LA(model, eval_loader, device):
    ...
    with torch.no_grad():
        for batch_idx, (batch_x, utt_list) in enumerate(eval_loader):
            ...
            # Get model output (no SAE/CPC loss during eval)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate CPC model on ASVspoof 2021 LA with torchaudio')
    ...
    print(f'✓ Loaded full state dict')
