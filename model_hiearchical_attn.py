...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int = 50):
        ...
        # Initialize with max_len, but will dynamically adjust if needed
    ...
    def forward(self, x: torch.Tensor):
        ...
        # If sequence length exceeds current positional embeddings, expand them
        if seq_len > self.pos_emb.size(1):
            # Create new positional embeddings
            new_pos_emb = nn.Parameter(torch.randn(1, seq_len, self.d_model, device=x.device) * 0.02)
            # Copy existing embeddings
            new_pos_emb.data[:, :self.pos_emb.size(1), :] = self.pos_emb.data
            self.pos_emb = new_pos_emb
        
        # Use only the required sequence length
